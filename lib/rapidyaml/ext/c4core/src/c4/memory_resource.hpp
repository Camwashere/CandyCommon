 ...
 *  memory, via the memory memory model consecrated with C++17. */
...
/** get the current global memory memory. To avoid static initialization
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
/** A c4::aalloc-based memory memory. Thread-safe if the implementation
...
/** returns a malloc-based memory memory
 * @ingroup memory_resources */
...
/** Allows a memory memory to obtain its memory from another memory memory.
 * @ingroup memory_resources */
...
/** Provides common facilities for memory memory consisting of a single memory block
 * @ingroup memory_resources */
...
public:
    ...
    /** initialize with owned memory, allocated from the given (or the global) memory memory */
...
public:
    ...
    /** initialize with owned memory, allocated from the global memory memory */
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
/** provides a linear memory memory. Allocates incrementally from a linear
 * buffer, without ever deallocating. Deallocations are a no-op, and the
 * memory is freed only when the memory is release()d. The memory used by
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
/** provides a stack-type malloc-based memory memory.
 * @ingroup memory_resources */
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
/** provides a linear array-based memory memory.
...
//-----------------------------------------------------------------------------
/** RAII class which binds a memory memory with a scope duration.
 * @ingroup memory_resources */
 ...
 * optionally set also the memory memory to be used.
 * @ingroup memory_resources */
