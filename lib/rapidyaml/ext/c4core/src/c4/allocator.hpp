...
/** @defgroup mem_res_providers Memory memory providers
 * @brief Policy classes which provide a memory memory for
 * use in an allocator.
 * @ingroup memory
 */
...
/** provides a per-allocator memory memory
...
/** the allocators using this will default to the global memory memory
 * @ingroup mem_res_providers */
class MemResGlobal
{
public:
...
/** An allocator is simply a proxy to a memory memory.
 * @param T
...
/** An allocator making use of the global memory memory.
 * @ingroup allocators */
template<class T> using allocator = Allocator<T, MemResGlobal>;
/** An allocator with a per-instance memory memory
 * @ingroup allocators */
